...
import AboutPageEditor from '@app/admin/pages/components/AboutPageEditor';
...
function readAboutData(value: unknown): Record<string, any> {
  ...
  // Manual/demo expert answers are no longer used. Public Q&A comes from ContactRequest.
  const cleanExperts = { ...experts };
  delete cleanExperts.answers;

  return {
    ...source,
    experts: cleanExperts,
  };
}
...
export default function PageForm({ page, onSubmit }: PageFormProps) {
  ...
  useEffect(() => {
    if (page) {
      ...
      setAboutData(readAboutData((page as Page & { custom_data?: unknown }).custom_data));
  ...
  const handleFormSubmit = async (data: FormData) => {
    ...
    if (!isAboutPage && (isLoading||isLoadingCategoryCards||isLoadingFaqs||isLoadingCasinos||isLoadingCasinoOptions||isLoadingCards||isLoadingSlotCards||isLoadingIconCards)) return <div>Loading...</div>;
