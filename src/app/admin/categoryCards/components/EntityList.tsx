...
export default function EntityList() {
    const [data, setData] = useState<ApiResponse<CategoryCard> | null>(null);
    ...
    const fetchData = async () => {
        ...
        try {
            const res = await fetch(`/api/admin/categoryCards?page=${page}&limit=${limit}`);
            if (!res.ok) throw new Error(`Error: ${res.status}`);
            const json: ApiResponse<CategoryCard> = await res.json();
            setData(json);
        } catch (err: unknown) {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await fetch(`/api/admin/categoryCards/${id}`, {
                method: 'DELETE',
            });
    ...
    return (
        <div className="p-6">
            <button
                onClick={() => router.push('/admin/categoryCards/create')}
                        ...
                        {data.data.map((categoryCard) => (
                            <tr key={categoryCard.id} className="border-b hover:bg-gray-50">
                                ...
                                <td className="p-3 flex gap-2">
                                    <button
                                        onClick={() => router.push(`/admin/categoryCards/${categoryCard.id}/edit`)}
